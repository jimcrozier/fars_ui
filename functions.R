...
model_fn = function(dataIn, varsinmodel,todummies,holdout,model_in){
  ...
  data_in_train = data.frame(dataIn[train_dati,], pred = predict(model, newdata = dataIn[train_dati,]))
  data_in_val = data.frame(dataIn[-train_dati,],pred = predict(model, newdata = dataIn[-train_dati,]))
